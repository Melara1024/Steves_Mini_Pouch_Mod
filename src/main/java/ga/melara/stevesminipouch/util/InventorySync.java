...
public class InventorySync {
    public static void initClient(InventoryStatsData data) {
        ((ICustomInventory) Minecraft.getInstance().player.inventory).initMiniPouch(data);
    }

    public static void syncEffectSizeToClient(int newEffectSize) {
        // Client-side effect slots are handled here
        ((ICustomInventory) Minecraft.getInstance().player.inventory).changeEffectSize(newEffectSize);
    }
}
